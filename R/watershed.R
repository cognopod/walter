...
watershed <- function(data, maxlatent = ncol(data), minlatent = 1, cutoff = 0.4) {
  ...
  COUNTITER <- 0
  for (i in maxlatent:minlatent) {
    COUNTITER <- COUNTITER + 1
    data.pa.oblimin <- psych::fa(data, nfactors=i, SMC=TRUE, fm="pa", rotate="oblimin", max.iter=100)
    FACTORTEST[,COUNTITER+1] <- as.vector(apply(data.pa.oblimin$Structure,1,function(x) {if(max(abs(x)) >= cutoff) paste0(LETTERS[i], which.max(abs(x))) else NA}))
  }
  return(FACTORTEST)
}
